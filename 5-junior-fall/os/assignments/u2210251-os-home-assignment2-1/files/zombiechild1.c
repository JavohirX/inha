...
printf("* OPEN ANOTHER TERMINAL KILL ME BY GETTING MY PID using ps -l AND THEN EXECUTE THE COMMAND kill -9 mypid *\n");
 printf("* NOW I HAVE BECOME A ZOMBIE(DEFUNCT) PROCESS - CHECK MY STATUSBY USING THE COMMAND ps -la *\n");
 printf("* IF YOU WANT TO KILL ALL ZOMBIE PROCESSES THEN KILL THE PARENTPROCESS BY USING kill -9 parentpid AND CHECK USING ps -la *\n");
printf("*******************************************************************************************************************************\n");
pause();
exit(0);/*CHILD*/ } }
